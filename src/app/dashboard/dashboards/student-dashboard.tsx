...
import { isSameDay, isWeekend, startOfMonth, endOfMonth, eachDayOfInterval, format, getDay, isBefore, differenceInCalendarDays, subDays, startOfWeek, endOfWeek } from 'date-fns';
  ...
  useEffect(() => {
    ...
    const { presentDays, absentDays, presentDaysForCalendar } = useMemo(() => {
     ...
     const { currentStreak, recordStreak, weeklyAttendance } = useMemo(() => {
        ...
        const lastAttendedDayIndex = sortedUniqueDaysForCurrentStreak.findIndex(d => d === todayStr || d === format(subDays(today, 1), 'yyyy-MM-dd'));

        if (lastAttendedDayIndex !== -1 && (sortedUniqueDaysForCurrentStreak[lastAttendedDayIndex] === todayStr || sortedUniqueDaysForCurrentStreak[lastAttendedDayIndex] === format(subDays(new Date(),1), 'yyyy-MM-dd') ) ) {
            streak = 1;
